...
def change_space_underscore(line):
    lr=''
    for ii in line:
        if ii == ' ':
            lr+='_'
...
def index(request):
    ...
    for topic in ll:
        ...
        for op in rr_qs:
            br1=Person()
            br1.name=change_space_underscore(op.text)
            br1.lbl=op.text
            l_op.append(br1)
        shuffle(l_op)
        ll_op.append(l_op)
...
def index1(request):
    if request.method == "POST":
        ...
        for topic in ll:
            ...
            for op in rr_qs:
                if op.correct:
                    s_op_n.add(change_space_underscore(op.text))
        for ii in s_op_n:
            name2+=(' '+ str(ii))
            #name2=name2[2:]
        ...
        if (len(ss & s_op_n) == len(s_op_n) and (len(ss | s_op_n) == len(s_op_n)) and len(ss)>0):
            ...
            print((len(ss & s_op_n) == len(s_op_n),(len(ss | s_op_n) == len(s_op_n)),len(ss)>0))
